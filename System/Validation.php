...
/**
 * Validation - A fast, extensible PHP input validation class.
...
class Validation {
    ...
    // ** ------------------------- Validation Data ------------------------------- ** //
    public $basic_tags = '<br><p><a><strong><b><i><em><img><blockquote><code><dd><dl><hr>
            <h1><h2><h3><h4><h5><h6><label><ul><li><span><sub><sup>';
    // ** --------------------------------Noise words   --------------------------** //
     ...
     */
    public function is_valid(array $data, array $validators) {
        $this->validation_rules($validators);

        if ($this->run($data) === false) {
            return $this->get_readable_errors(false);
        } else {
            return true;
        }
    }
     ...
     * @return mixed
     */
    public function filter_input(array $data, array $filters) {
                    ...
                    if (is_callable(array($this, $method))) {
                        ...
                        if ($result === false) {
                            $this->errors[] = array(
                                'field' => $field,
                                'value' => $input,
                                'rule' => $this->validation_methods[$rule],
                                'param' => $param,
                            );
                        }
                    } else {
     ...
     */
    public function set_field_name($field, $readable_name) {
        $this->fields[$field] = $readable_name;
    }
     ...
     * @param array $array
     */
    public function set_field_names(array $array) {
    ...
    public function get_readable_errors() {
        ...
        foreach ($this->errors as $e) {
            ...
            if (array_key_exists($e['field'], $this->fields)) {
                $field = $this->fields[$e['field']];
            }
    ...
    protected function filter_noise_words($value, $params = null) {
        ...
        $value = " $value ";
        $words = explode(',', $this->en_noise_words);
    ...
    protected function filter_basic_tags($value, $params = null) {
        return strip_tags($value, $this->basic_tags);
    }
     ...
     *
     * See: https://github.com/Wixel/Validation/issues/5
